...
const Jersey3D = forwardRef(({
  ...
  const updateTexture = () => {
    ...
    if (nameText || numberText) {
      ...
      // Draw number
      if (numberText) {
        const numberSize = fontSize * 1.8;
        ctx.font = `bold ${numberSize}px ${fontFamily}, Arial, sans-serif`;
        ctx.lineWidth = 3;
        ctx.strokeText(numberText, centerX, numberY);
        ctx.fillText(numberText, centerX, numberY);
      }
